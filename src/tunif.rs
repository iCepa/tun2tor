...
impl TunIf {
    ...
    pub fn input_packet(&self, packet: &[u8]) {
        ...
        if result.is_err() {
            return println!("Error: IP header parsing failed: {:?}",
                            result.err().unwrap());
        }

        let header = result.unwrap();
        if header.len() > packet.len() {
            return println!("Error: IP header length is greater than total packet length, packet \
                             dropped");
        }
                ...
                match UdpHeader::from_bytes(payload) {
                    Ok(udp_hdr) => {
                        ...
                        if udp_hdr.checksum_valid(&data, &header) == false {
                            return println!("Error: UDP header checksum is invalid, packet \
                                             dropped");
                        }
