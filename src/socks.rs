    ...
    fn build(&self,
        ...
        let handshake = greeting.and_then(move |(stream, _)| {
            read_exact(stream, vec![0; 2]).and_then(move |(stream, resp)| {
                ...
                match resp[1] {
                    0 => Ok(stream),
                    0xFF => Err(io::Error::new(io::ErrorKind::Other, "no acceptable auth methods")),
                    _ => Err(io::Error::new(io::ErrorKind::InvalidData, "unknown auth method")),
                }
            })
        });
        ...
        let command = handshake.and_then(move |stream| {
            ...
            (&mut buf[len - 2..]).write_u16::<NetworkEndian>(addr.port()).unwrap();
